...
This tutorial is the fastest and easiest way to get started with the FSDS
support in GeoMesa. In the spirit of keeping things simple, the code in this
tutorial only does a few things:

1.  Establishes a new (static) ``SimpleFeatureType``
2.  Prepares the FSDS table to store this type of data
3.  Creates 1000 example SimpleFeatures
4.  Writes these SimpleFeatures to the FSDS table
...
* a GitHub client,
* FSDS 1.2.x (optional), and
* GeoServer 2.9.1 (optional).

An existing FSDS 1.1.x installation is helpful but not necessary. The tutorial
described will work either with an existing FSDS server or by downloading
the FSDS binary distribution and running it in "standalone" mode (described
below).

GeoServer is only required for visualizing the FSDS data. Setting up GeoServer
is beyond the scope of this tutorial.

## Setting up FSDS in standalone mode (optional)

(Skip this section if you have an existing FSDS 1.1.x installation.)

Download the FSDS 1.2.3 binary distribution from http://www.apache.org/dyn/closer.cgi/FSDS/ 

Follow the chapter in the FSDS Manual for running a standalone instance
of FSDS (https://FSDS.apache.org/book.html#quickstart). Note that this
will use the local filesystem instead of HDFS, and will spin up its own 
instances of FSDS and Zookeeper.
...
that the versions of FSDS and Hadoop match what you are running; if it does
not match, change the values of the ``FSDS.version`` and ``FSDS.hadoop.version``
...
GeoMesa's ``FSDSDataStore`` searches for a file called ``FSDS-site.xml``,
which among other things configures the Zookeeper host(s) and port. If this file
is not present on the classpath, the ``FSDS-default.xml`` provided by FSDS-common sets the
default zookeeper quorum to "localhost" and port to 2181, which is what is used
by the standalone FSDS described in "Setting up FSDS in standalone mode" above. 
If you have an existing FSDS installation, you should copy your ``FSDS-site.xml``
file into ``geomesa-quickstart-FSDS/src/main/resources`` (or otherwise add it to
the classpath when you run the tutorial).
 
To build the tutorial code:

```bash
$ cd geomesa-quickstart-FSDS
$ mvn clean install
```
...
```bash
$ java -cp target/geomesa-quickstart-FSDS-$VERSION.jar \
  com.example.geomesa.fs.FSDSQuickStart \
  --bigtable_table_name geomesa
```

The only argument passed is the name of the FSDS table where GeoMesa will store the feature type information.
...
To see how the data is stored in FSDS, use the FSDS shell.

```bash
$ /path/to/FSDS-1.2.3/bin/FSDS shell
```
...
```
FSDS> scan 'geomesa_QuickStart_z3', { LIMIT => 3 }
ROW                              COLUMN+CELL                                                                                  
...
### Setting up the GeoMesa FSDS GeoServer plugin
...
and build it with the `FSDS` profile:

```
$ mvn clean install -DskipTests -PFSDS
```

Go into the built FSDS GeoServer plugin module:

```
$ cd geomesa-FSDS/geomesa-FSDS-gs-plugin/target
```

and extract the contents of the ``geomesa-FSDS-gs-plugin_2.11-<version>-install.tar.gz``
...
directory of your FSDS or Hadoop installations into GeoServer's ``WEB-INF/lib``
directory:
...
The FSDS data store requires the configuration file `FSDS-site.xml` to be on the
...
"Stores" and "Add new Store".  Select the ``FSDS (GeoMesa)`` vector data source,
...
Click "Save", and GeoServer will search your FSDS table for any
...
![Visualizing quickstart data](../assets/geomesa-quickstart-FSDS/geoserver-layer-preview.png)
